...
The proxy also refuses, in the argument array, the global options that would undo any of that — `-c` and `--config-env`, which set configuration outranking the pins; `--exec-path`, which chooses where git looks for the program to run; `--git-dir` and `--work-tree`, which name a repository directly and so sidestep the workspace containment applied to the working directory; and `--global` and `--system`, which write the configuration files the proxy pins. `-C` remains available for choosing a directory inside a leased workspace, since the containment check follows it, and repository-local `git config` remains available because that is how a clone's commit identity is set. Refused alongside them are the subcommands whose purpose is to run a caller-named command, among them `bisect`, `difftool`, `mergetool`, `filter-branch` and `send-email`. Every refusal comes back as `SECURITY_POLICY_BLOCKED` with rule `git.argument.refused`, and no shipped skill uses any of them.

These pins do not extend to configuration stored inside a repository's own `.git/config`, which the agent authors and which `git` honors for some settings that name a program to run. Reducing that surface is why the proxy is moving toward receiving file content from the agent rather than operating inside a directory the agent controls; until then, treat a cloned working tree as agent-controlled input rather than as a trust boundary.
