...
| `docs/designs/capability-envelope-write-path.md` | Feature design | Open question behind architecture 09: the capability's read path can be bound to a request but its write path is proved by a per-agent NATS subject prefix, so a hop can descend from a concurrent request's root. | Write-path binding, per-request bus identity, entry lifetime, revocation actor | Open question; input for the downscoping design session, not a decision |
