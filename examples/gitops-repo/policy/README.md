...
harness itself, installed by both supported paths:

- the Helm chart, as `charts/kube-agents/templates/agent-rbac-admission-policy.yaml` (turn off with
  `admissionPolicy.enabled=false`);
- the provisioning scripts, applied by `provision_03_gcp_gke_operator.sh`.
